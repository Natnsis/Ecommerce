...
const Cart = () => {
    ...
    useEffect(() => {
        const fetchCart = async () => {
            try {
                ...
                console.log("Cart items fetched from backend:", response.data.cartItems); // Debugging log
    ...
    return (
        <div className="p-5">
            <h1 className="text-2xl font-bold mb-5 text-center">{translations.yourCart}</h1>
            {cart.length === 0 ? (
                    ...
                    {cart.map((item) => (
                        ...
                        >
                            <div className="text-center md:text-left">
                                ...
                                <p>{translations.price}: {item.price} USD</p>
                                <p>{translations.quantity}: {item.quantity}</p>
                                <p className={`font-semibold ${item.payed ? "text-green-500" : "text-yellow-500"}`}>
                                    {translations.status}: {item.payed ? translations.paid : translations.pending}
                                </p>
                            </div>
                            <button
